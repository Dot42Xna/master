...
//
// Original filename: Java.Util.Concurrent.cs
//
...
namespace Java.Util.Concurrent
{
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>A task that returns a result and may throw an exception. Implementors define a single method with no arguments called <code>call</code>.</para><para>The <code>Callable</code> interface is similar to java.lang.Runnable, in that both are designed for classes whose instances are potentially executed by another thread. A <code>Runnable</code>, however, does not return a result and cannot throw a checked exception.</para><para>The Executors class contains utility methods to convert from other common forms to <code>Callable</code> classes.</para><para><para>Executor </para><para>1.5 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Inserts the specified element into this queue if it is possible to do so immediately without violating capacity restrictions, returning <code>true</code> upon success and <code>false</code> if no space is currently available. When using a capacity-restricted queue, this method is generally preferable to add, which can fail to insert an element only by throwing an exception.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if successful, or <code>false</code> if the specified waiting time elapses before space is available </para>
				/// </returns>
				/// <java-name>
				/// offer
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>the head of this queue, or <code>null</code> if the specified waiting time elapses before an element is available </para>
				/// </returns>
				/// <java-name>
				/// poll
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns the number of additional elements that this queue can ideally (in the absence of memory or resource constraints) accept without blocking, or <code>Integer.MAX_VALUE</code> if there is no intrinsic limit.</para><para>Note that you <b>cannot</b> always tell if an attempt to insert an element will succeed by inspecting <code>remainingCapacity</code> because it may be the case that another thread is about to insert or remove an element.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this queue contains the specified element. More formally, returns <code>true</code> if and only if this queue contains at least one element <code>e</code> such that <code>o.equals(e)</code>.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this queue contains the specified element </para>
				/// </returns>
				/// <java-name>
				/// contains
				/// </java-name>
				...
				/// <summary>
				/// <para>Removes all available elements from this queue and adds them to the given collection. This operation may be more efficient than repeatedly polling this queue. A failure encountered while attempting to add elements to collection <code>c</code> may result in elements being in neither, either or both collections when the associated exception is thrown. Attempts to drain a queue to itself result in <code>IllegalArgumentException</code>. Further, the behavior of this operation is undefined if the specified collection is modified while the operation is in progress.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Removes at most the given number of available elements from this queue and adds them to the given collection. A failure encountered while attempting to add elements to collection <code>c</code> may result in elements being in neither, either or both collections when the associated exception is thrown. Attempts to drain a queue to itself result in <code>IllegalArgumentException</code>. Further, the behavior of this operation is undefined if the specified collection is modified while the operation is in progress.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the number of elements transferred </para>
				/// </returns>
 ...
 /* scope: __dot42__ */ 
    ...
    "p;", AccessFlags = 1025, Signature = "(TK;TK;)Ljava/util/concurrent/ConcurrentNavigableMap<TK;TV;>;")]
		...
		/// <summary>
		/// <para>A service that decouples the production of new asynchronous tasks from the consumption of the results of completed tasks. Producers <code>submit</code> tasks for execution. Consumers <code>take</code> completed tasks and process their results in the order they complete. A <code>CompletionService</code> can for example be used to manage asynchronous IO, in which tasks that perform reads are submitted in one part of a program or system, and then acted upon in a different part of the program when the reads complete, possibly in a different order than they were requested.</para><para>Typically, a <code>CompletionService</code> relies on a separate Executor to actually execute the tasks, in which case the <code>CompletionService</code> only manages an internal completion queue. The ExecutorCompletionService class provides an implementation of this approach.</para><para>Memory consistency effects: Actions in a thread prior to submitting a task to a <c> CompletionService </c>  actions taken by that task, which in turn <b>happen-before</b> actions following a successful return from the corresponding <c> take() </c> . </para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Retrieves and removes the Future representing the next completed task or <code>null</code> if none are present.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the Future representing the next completed task, or <code>null</code> if none are present </para>
				/// </returns>
				/// <java-name>
				/// poll
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>the Future representing the next completed task or <code>null</code> if the specified waiting time elapses before one is present </para>
				/// </returns>
				/// <java-name>
				/// poll
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Constructs a <code>CancellationException</code> with no detail message. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>Constructs a <code>CancellationException</code> with the specified detail message.</para><para></para>        
				/// </summary>
		...
		/// <summary>
		/// <para>A  BlockingQueue blocking queue} in which each insert operation must wait for a corresponding remove operation by another thread, and vice versa. A synchronous queue does not have any internal capacity, not even a capacity of one. You cannot <code>peek</code> at a synchronous queue because an element is only present when you try to remove it; you cannot insert an element (using any method) unless another thread is trying to remove it; you cannot iterate as there is nothing to iterate. The <b>head</b> of the queue is the element that the first queued inserting thread is trying to add to the queue; if there is no such queued thread then no element is available for removal and <code>poll()</code> will return <code>null</code>. For purposes of other <code>Collection</code> methods (for example <code>contains</code>), a <code>SynchronousQueue</code> acts as an empty collection. This queue does not permit <code>null</code> elements.</para><para>Synchronous queues are similar to rendezvous channels used in CSP and Ada. They are well suited for handoff designs, in which an object running in one thread must sync up with an object running in another thread in order to hand it some information, event, or task.</para><para>This class supports an optional fairness policy for ordering waiting producer and consumer threads. By default, this ordering is not guaranteed. However, a queue constructed with fairness set to <code>true</code> grants threads access in FIFO order.</para><para>This class and its iterator implement all of the <b>optional</b> methods of the Collection and Iterator interfaces.</para><para><para>1.5 </para><para>Doug Lea and Bill Scherer and Michael Scott </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Creates a <code>SynchronousQueue</code> with nonfair access policy. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>Creates a <code>SynchronousQueue</code> with the specified fairness policy.</para><para></para>        
				/// </summary>
				...
				/// </java-name>
				[Dot42.DexImport("put", "(Ljava/lang/Object;)V", AccessFlags = 1, Signature = "(TE;)V")]
				public virtual void Put(E o) /* MethodBuilder.Create */ 
				{
				}
				...
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if successful, or <code>false</code> if the specified waiting time elapses before a consumer appears. </para>
				/// </returns>
				/// <java-name>
				/// offer
				/// </java-name>
				[Dot42.DexImport("offer", "(Ljava/lang/Object;JLjava/util/concurrent/TimeUnit;)Z", AccessFlags = 1, Signature = "(TE;JLjava/util/concurrent/TimeUnit;)Z")]
				public virtual bool Offer(E o, long timeout, global::Java.Util.Concurrent.TimeUnit unit) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
				...
				/// </summary>
				/// <returns>
				/// <para>the head of this queue, or <code>null</code> if the specified waiting time elapses before an element is present. </para>
				/// </returns>
				/// <java-name>
				/// poll
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>the head of this queue, or <code>null</code> if no element is available. </para>
				/// </returns>
				/// <java-name>
				/// poll
				/// </java-name>
				...
				/// <summary>
				/// <para>Always returns <code>true</code>. A <code>SynchronousQueue</code> has no internal capacity.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Always returns zero. A <code>SynchronousQueue</code> has no internal capacity.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>zero. </para>
				/// </returns>
				...
				/// <summary>
				/// <para>Does nothing. A <code>SynchronousQueue</code> has no internal capacity. </para>        
				/// </summary>
				/// <java-name>
				/// clear
				/// </java-name>
				...
				/// <summary>
				/// <para>Always returns <code>false</code>. A <code>SynchronousQueue</code> has no internal capacity.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Returns <code>false</code> unless the given collection is empty. A <code>SynchronousQueue</code> has no internal capacity.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>false</code> unless given collection is empty </para>
				/// </returns>
				/// <java-name>
				/// containsAll
				/// </java-name>
				...
				/// <summary>
				/// <para>Always returns <code>null</code>. A <code>SynchronousQueue</code> does not return elements unless actively waited on.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>null</code> </para>
				/// </returns>
				/// <java-name>
				/// peek
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns an empty iterator in which <code>hasNext</code> always returns <code>false</code>.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Sets the zeroeth element of the specified array to <code>null</code> (if the array has non-zero length) and returns it.</para><para></para>        
				/// </summary>
				/// <returns>
		...
		/// <summary>
		/// <para>An ExecutorService that can schedule commands to run after a given delay, or to execute periodically.</para><para>The <code>schedule</code> methods create tasks with various delays and return a task object that can be used to cancel or check execution. The <code>scheduleAtFixedRate</code> and <code>scheduleWithFixedDelay</code> methods create and execute tasks that run periodically until cancelled.</para><para>Commands submitted using the Executor#execute and ExecutorService <code>submit</code> methods are scheduled with a requested delay of zero. Zero and negative delays (but not periods) are also allowed in <code>schedule</code> methods, and are treated as requests for immediate execution.</para><para>All <code>schedule</code> methods accept <b>relative</b> delays and periods as arguments, not absolute times or dates. It is a simple matter to transform an absolute time represented as a java.util.Date to the required form. For example, to schedule at a certain future <code>date</code>, you can use: <code>schedule(task, date.getTime() - System.currentTimeMillis(), TimeUnit.MILLISECONDS)</code>. Beware however that expiration of a relative delay need not coincide with the current <code>Date</code> at which the task is enabled due to network time synchronization protocols, clock drift, or other factors.</para><para>The Executors class provides convenient factory methods for the ScheduledExecutorService implementations provided in this package.</para><para><h3>Usage Example</h3></para><para>Here is a class with a method that sets up a ScheduledExecutorService to beep every ten seconds for an hour:</para><para><pre>    import static java.util.concurrent.TimeUnit.*;
		/// 
		///    class BeeperControl {
		/// 
 ...
 /* scope: __dot42__ */ 
    ...
    "/TimeUnit;)Ljava/util/concurrent/ScheduledFuture<TV;>;")]
				...
				/// <summary>
				/// <para>Creates and executes a periodic action that becomes enabled first after the given initial delay, and subsequently with the given period; that is executions will commence after <code>initialDelay</code> then <code>initialDelay+period</code>, then <code>initialDelay + 2 * period</code>, and so on. If any execution of the task encounters an exception, subsequent executions are suppressed. Otherwise, the task will only terminate via cancellation or termination of the executor. If any execution of this task takes longer than its period, then subsequent executions may start late, but will not concurrently execute.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>a ScheduledFuture representing pending completion of the task, and whose <code>get()</code> method will throw an exception upon cancellation </para>
				/// </returns>
				/// <java-name>
				/// scheduleAtFixedRate
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// </summary>
				/// <returns>
				/// <para>the previous value associated with the specified key, or <code>null</code> if there was no mapping for the key. (A <code>null</code> return can also indicate that the map previously associated <code>null</code> with the key, if the implementation supports null values.) </para><para>the previous value associated with the specified key, or <code>null</code> if there was no mapping for the key </para>
				/// </returns>
				/// <java-name>
				/// putIfAbsent
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if the value was removed </para>
				/// </returns>
				/// <java-name>
				/// remove
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if the value was replaced </para>
				/// </returns>
				/// <java-name>
				/// replace
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this set contains no elements.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this set contains no elements </para>
				/// </returns>
				/// <java-name>
				/// isEmpty
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this set contains the specified element. More formally, returns <code>true</code> if and only if this set contains an element <code>e</code> such that <code>(o==null ? e==null : o.equals(e))</code>.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this set contains the specified element </para>
				/// </returns>
				/// <java-name>
				/// contains
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this set contained the specified element </para>
				/// </returns>
				/// <java-name>
				/// remove
				/// </java-name>
				...
				/// <summary>
				/// <para>Adds the specified element to this set if it is not already present. More formally, adds the specified element <code>e</code> to this set if the set contains no element <code>e2</code> such that <code>(e==null ? e2==null : e.equals(e2))</code>. If this set already contains the element, the call leaves the set unchanged and returns <code>false</code>.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this set did not already contain the specified element </para>
				/// </returns>
				/// <java-name>
				/// add
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this set contains all of the elements of the specified collection. If the specified collection is also a set, this method returns <code>true</code> if it is a <b>subset</b> of this set.</para><para><para>contains(Object) </para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this set contains all of the elements of the specified collection </para>
				/// </returns>
				/// <java-name>
				/// containsAll
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
    ...
    "/Future<TT;>;")]
				...
				/// <summary>
				/// <para>Sets the policy on whether to continue executing existing periodic tasks even when this executor has been <c> shutdown </c> . In this case, these tasks will only terminate upon <c> shutdownNow </c> or after setting the policy to <c> false </c> when already shutdown. This value is by default <c> false </c> .</para><para><para>getContinueExistingPeriodicTasksAfterShutdownPolicy </para></para>        
				/// </summary>
				/// <java-name>
				/// setContinueExistingPeriodicTasksAfterShutdownPolicy
				/// </java-name>
				[Dot42.DexImport("setContinueExistingPeriodicTasksAfterShutdownPolicy", "(Z)V", AccessFlags = 1)]
				public virtual void SetContinueExistingPeriodicTasksAfterShutdownPolicy(bool value) /* MethodBuilder.Create */ 
				{
				}
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Inserts the specified element at the front of this deque if it is possible to do so immediately without violating capacity restrictions, throwing an <code>IllegalStateException</code> if no space is currently available. When using a capacity-restricted deque, it is generally preferable to use offerFirst.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// addFirst
				/// </java-name>
				...
				/// <summary>
				/// <para>Inserts the specified element at the end of this deque if it is possible to do so immediately without violating capacity restrictions, throwing an <code>IllegalStateException</code> if no space is currently available. When using a capacity-restricted deque, it is generally preferable to use offerLast.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// addLast
				/// </java-name>
				...
				/// <summary>
				/// <para>Inserts the specified element at the front of this deque if it is possible to do so immediately without violating capacity restrictions, returning <code>true</code> upon success and <code>false</code> if no space is currently available. When using a capacity-restricted deque, this method is generally preferable to the addFirst method, which can fail to insert an element only by throwing an exception.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// offerFirst
				/// </java-name>
				...
				/// <summary>
				/// <para>Inserts the specified element at the end of this deque if it is possible to do so immediately without violating capacity restrictions, returning <code>true</code> upon success and <code>false</code> if no space is currently available. When using a capacity-restricted deque, this method is generally preferable to the addLast method, which can fail to insert an element only by throwing an exception.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// offerLast
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>the head of this deque, or <code>null</code> if the specified waiting time elapses before an element is available </para>
				/// </returns>
				/// <java-name>
				/// pollFirst
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>the tail of this deque, or <code>null</code> if the specified waiting time elapses before an element is available </para>
				/// </returns>
				/// <java-name>
				/// pollLast
				/// </java-name>
				...
				/// <summary>
				/// <para>Inserts the specified element into the queue represented by this deque (in other words, at the tail of this deque) if it is possible to do so immediately without violating capacity restrictions, returning <code>true</code> upon success and throwing an <code>IllegalStateException</code> if no space is currently available. When using a capacity-restricted deque, it is generally preferable to use offer.</para><para>This method is equivalent to addLast.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// add
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this deque changed as a result of the call </para>
				/// </returns>
				/// <java-name>
				/// remove
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>A mix-in style interface for marking objects that should be acted upon after a given delay.</para><para>An implementation of this interface must define a <code>compareTo</code> method that provides an ordering consistent with its <code>getDelay</code> method.</para><para><para>1.5 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Creates a new <code>CyclicBarrier</code> that will trip when the given number of parties (threads) are waiting upon it, and does not perform a predefined action when the barrier is tripped.</para><para></para>        
				/// </summary>
				[Dot42.DexImport("<init>", "(I)V", AccessFlags = 1)]
		...
		/// <summary>
		/// <para>Exception thrown when a blocking operation times out. Blocking operations for which a timeout is specified need a means to indicate that the timeout has occurred. For many such operations it is possible to return a value that indicates timeout; when that is not possible or desirable then <code>TimeoutException</code> should be declared and thrown.</para><para><para>1.5 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Constructs a <code>TimeoutException</code> with no specified detail message. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>Constructs a <code>TimeoutException</code> with the specified detail message.</para><para></para>        
				/// </summary>
				[Dot42.DexImport("<init>", "(Ljava/lang/String;)V", AccessFlags = 1)]
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Returns a shallow copy of this <code>ConcurrentSkipListSet</code> instance. (The elements themselves are not cloned.)</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Returns the number of elements in this set. If this set contains more than <code>Integer.MAX_VALUE</code> elements, it returns <code>Integer.MAX_VALUE</code>.</para><para>Beware that, unlike in most collections, this method is <b>NOT</b> a constant-time operation. Because of the asynchronous nature of these sets, determining the current number of elements requires traversing them all to count them. Additionally, it is possible for the size to change during execution of this method, in which case the returned result will be inaccurate. Thus, this method is typically not very useful in concurrent applications.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the number of elements in this set </para>
				/// </returns>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this set contains no elements. </para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this set contains no elements </para>
				/// </returns>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this set contains the specified element. More formally, returns <code>true</code> if and only if this set contains an element <code>e</code> such that <code>o.equals(e)</code>.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this set contains the specified element </para>
				/// </returns>
				...
				/// <summary>
				/// <para>Compares the specified object with this set for equality. Returns <code>true</code> if the specified object is also a set, the two sets have the same size, and every member of the specified set is contained in this set (or equivalently, every member of this set is contained in the specified set). This definition ensures that the equals method works properly across different implementations of the set interface.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if the specified object is equal to this set </para>
				/// </returns>
				/// <java-name>
				/// equals
				/// </java-name>
				...
				[Dot42.DexImport("java/util/SortedSet", "headSet", "(Ljava/lang/Object;)Ljava/util/SortedSet;", AccessFlags = 1025, Signature = "(TE;)Ljava/util/SortedSet<TE;>;")]
				global::Java.Util.ISortedSet<E> global::Java.Util.ISortedSet<E>.HeadSet(E end) /* TypeBuilder.AddAbstractInterfaceMethods */ 
				{
						return default(global::Java.Util.ISortedSet<E>);
				}

				[Dot42.DexImport("java/util/SortedSet", "subSet", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/util/SortedSet;", AccessFlags = 1025, Signature = "(TE;TE;)Ljava/util/SortedSet<TE;>;")]
				global::Java.Util.ISortedSet<E> global::Java.Util.ISortedSet<E>.SubSet(E start, E end) /* TypeBuilder.AddAbstractInterfaceMethods */ 
				{
						return default(global::Java.Util.ISortedSet<E>);
				}

				[Dot42.DexImport("java/util/SortedSet", "tailSet", "(Ljava/lang/Object;)Ljava/util/SortedSet;", AccessFlags = 1025, Signature = "(TE;)Ljava/util/SortedSet<TE;>;")]
				global::Java.Util.ISortedSet<E> global::Java.Util.ISortedSet<E>.TailSet(E start) /* TypeBuilder.AddAbstractInterfaceMethods */ 
				{
						return default(global::Java.Util.ISortedSet<E>);
				}
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Creates a thread pool that reuses a fixed number of threads operating off a shared unbounded queue, using the provided ThreadFactory to create new threads when needed. At any point, at most <code>nThreads</code> threads will be active processing tasks. If additional tasks are submitted when all threads are active, they will wait in the queue until a thread is available. If any thread terminates due to a failure during execution prior to shutdown, a new one will take its place if needed to execute subsequent tasks. The threads in the pool will exist until it is explicitly shutdown.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the newly created thread pool </para>
				/// </returns>
				...
				/// <summary>
				/// <para>Creates an Executor that uses a single worker thread operating off an unbounded queue. (Note however that if this single thread terminates due to a failure during execution prior to shutdown, a new one will take its place if needed to execute subsequent tasks.) Tasks are guaranteed to execute sequentially, and no more than one task will be active at any given time. Unlike the otherwise equivalent <code>newFixedThreadPool(1)</code> the returned executor is guaranteed not to be reconfigurable to use additional threads.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Creates an Executor that uses a single worker thread operating off an unbounded queue, and uses the provided ThreadFactory to create a new thread when needed. Unlike the otherwise equivalent <code>newFixedThreadPool(1, threadFactory)</code> the returned executor is guaranteed not to be reconfigurable to use additional threads.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the newly created single-threaded Executor </para>
				/// </returns>
				...
				/// <summary>
				/// <para>Creates a single-threaded executor that can schedule commands to run after a given delay, or to execute periodically. (Note however that if this single thread terminates due to a failure during execution prior to shutdown, a new one will take its place if needed to execute subsequent tasks.) Tasks are guaranteed to execute sequentially, and no more than one task will be active at any given time. Unlike the otherwise equivalent <code>newScheduledThreadPool(1)</code> the returned executor is guaranteed not to be reconfigurable to use additional threads. </para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Creates a single-threaded executor that can schedule commands to run after a given delay, or to execute periodically. (Note however that if this single thread terminates due to a failure during execution prior to shutdown, a new one will take its place if needed to execute subsequent tasks.) Tasks are guaranteed to execute sequentially, and no more than one task will be active at any given time. Unlike the otherwise equivalent <code>newScheduledThreadPool(1, threadFactory)</code> the returned executor is guaranteed not to be reconfigurable to use additional threads. </para>        
				/// </summary>
				/// <returns>
    ...
    "vice;", AccessFlags = 9)]
				...
				/// </summary>
				/// <returns>
				/// <para>an <code>ExecutorService</code> instance </para>
				/// </returns>
				/// <java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>a <code>ScheduledExecutorService</code> instance </para>
				/// </returns>
				/// <java-name>
    ...
    "xecutorService;", AccessFlags = 9)]
				...
				/// <summary>
				/// <para>Returns a default thread factory used to create new threads. This factory creates all new threads used by an Executor in the same ThreadGroup. Each new thread is created as a non-daemon thread with priority set to the smaller of <code>Thread.NORM_PRIORITY</code> and the maximum priority permitted in the thread group. New threads have names accessible via Thread#getName of <b>pool-N-thread-M</b>, where <b>N</b> is the sequence number of this factory, and <b>M</b> is the sequence number of the thread created by this factory. </para>        
				/// </summary>
				/// <returns>
    ...
    "/Callable<TT;>;")]
				...
				/// <summary>
				/// <para>Returns a Callable object that will, when called, execute the given <code>callable</code> under the current with the current context class loader as the context class loader.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>a callable object </para>
				/// </returns>
 ...
 /* scope: __dot42__ */ 
		{
				...
				public virtual void Add(int index, E e) /* MethodBuilder.Create */ 
				{
				}
				...
				public virtual bool AddAll(int index, global::Java.Util.ICollection<E> collection) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
				...
				/// addAllAbsent
				/// </java-name>
				[Dot42.DexImport("addAllAbsent", "(Ljava/util/Collection;)I", AccessFlags = 1, Signature = "(Ljava/util/Collection<+TE;>;)I")]
				...
				/// addIfAbsent
				/// </java-name>
				[Dot42.DexImport("addIfAbsent", "(Ljava/lang/Object;)Z", AccessFlags = 1, Signature = "(TE;)Z")]
				...
				/// </java-name>
				[Dot42.DexImport("contains", "(Ljava/lang/Object;)Z", AccessFlags = 1)]
				public virtual bool Contains(object o) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
				...
				public virtual E Get(int index) /* MethodBuilder.Create */ 
				{
						return default(E);
				}
				...
				public virtual E Remove(int index) /* MethodBuilder.Create */ 
				{
						return default(E);
				}
				...
				/// </java-name>
				[Dot42.DexImport("remove", "(Ljava/lang/Object;)Z", AccessFlags = 1)]
				public virtual bool Remove(object index) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
				...
				public virtual E Set(int index, E e) /* MethodBuilder.Create */ 
				{
						return default(E);
				}
				...
				public virtual global::Java.Util.IList<E> SubList(int from, int to) /* MethodBuilder.Create */ 
				{
				...
				/// </java-name>
				[Dot42.DexImport("toArray", "([Ljava/lang/Object;)[Ljava/lang/Object;", AccessFlags = 1, Signature = "<T:Ljava/lang/Object;>([TT;)[TT;")]
				public virtual T[] ToArray<T>(T[] contents) /* MethodBuilder.Create */ 
				{
						return default(T[]);
				}
		...
		/// <summary>
		/// <para>Provides default implementations of ExecutorService execution methods. This class implements the <code>submit</code>, <code>invokeAny</code> and <code>invokeAll</code> methods using a RunnableFuture returned by <code>newTaskFor</code>, which defaults to the FutureTask class provided in this package. For example, the implementation of <code>submit(Runnable)</code> creates an associated <code>RunnableFuture</code> that is executed and returned. Subclasses may override the <code>newTaskFor</code> methods to return <code>RunnableFuture</code> implementations other than <code>FutureTask</code>.</para><para><b>Extension example</b>. Here is a sketch of a class that customizes ThreadPoolExecutor to use a <code>CustomTask</code> class instead of the default <code>FutureTask</code>: <pre>    public class CustomThreadPoolExecutor extends ThreadPoolExecutor {
		/// 
		///   
		/// 
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Returns a <code>RunnableFuture</code> for the given runnable and default value.</para><para><para>1.6 </para></para>        
				/// </summary>
				/// <returns>
				/// <para>a <code>RunnableFuture</code> which when run will run the underlying runnable and which, as a <code>Future</code>, will yield the given value as its result and provide for cancellation of the underlying task. </para>
				/// </returns>
				/// <java-name>
				/// newTaskFor
				/// </java-name>
    ...
    "re<TT;>;")]
				...
				/// <summary>
				/// <para>Returns a <code>RunnableFuture</code> for the given callable task.</para><para><para>1.6 </para></para>        
				/// </summary>
				/// <returns>
				/// <para>a <code>RunnableFuture</code> which when run will call the underlying callable and which, as a <code>Future</code>, will yield the callable's result as its result and provide for cancellation of the underlying task. </para>
				/// </returns>
				/// <java-name>
				/// newTaskFor
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>A <code>Future</code> represents the result of an asynchronous computation. Methods are provided to check if the computation is complete, to wait for its completion, and to retrieve the result of the computation. The result can only be retrieved using method <code>get</code> when the computation has completed, blocking if necessary until it is ready. Cancellation is performed by the <code>cancel</code> method. Additional methods are provided to determine if the task completed normally or was cancelled. Once a computation has completed, the computation cannot be cancelled. If you would like to use a <code>Future</code> for the sake of cancellability but not provide a usable result, you can declare types of the form <c> Future&lt;?&gt; </c> and return <code>null</code> as a result of the underlying task.</para><para><b>Sample Usage</b> (Note that the following classes are all made-up.) </para><para><pre>    interface ArchiveSearcher { String search(String target); }
		/// 
		///    class App {
		/// 
		...
		///    } 
		/// 
		///  </pre></para><para>The FutureTask class is an implementation of <code>Future</code> that implements <code>Runnable</code>, and so may be executed by an <code>Executor</code>. For example, the above construction with <code>submit</code> could be replaced by: <pre>        FutureTask&lt;String&gt; future =
		/// 
		...
		/// 
		///  </pre></para><para>Memory consistency effects: Actions taken by the asynchronous computation  actions following the corresponding <c> Future.get() </c> in another thread.</para><para><para>FutureTask </para><simplesectsep></simplesectsep><para>Executor </para><para>1.5 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Constructs a <code>RejectedExecutionException</code> with no detail message. The cause is not initialized, and may subsequently be initialized by a call to initCause. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>Constructs a <code>RejectedExecutionException</code> with the specified detail message and cause.</para><para></para>        
				/// </summary>
 ...
 /* scope: __dot42__ */ 
		...
		///    } 
		/// 
		///  </pre></para><para>The following method shuts down an <code>ExecutorService</code> in two phases, first by calling <code>shutdown</code> to reject incoming tasks, and then calling <code>shutdownNow</code>, if necessary, to cancel any lingering tasks:</para><para><pre>    void shutdownAndAwaitTermination(ExecutorService pool) {
		/// 
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this executor has been shut down.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this executor has been shut down </para>
				/// </returns>
				/// <java-name>
				/// isShutdown
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if all tasks have completed following shut down. Note that <code>isTerminated</code> is never <code>true</code> unless either <code>shutdown</code> or <code>shutdownNow</code> was called first.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if all tasks have completed following shut down </para>
				/// </returns>
				/// <java-name>
				/// isTerminated
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this executor terminated and <code>false</code> if the timeout elapsed before termination </para>
				/// </returns>
				/// <java-name>
				/// awaitTermination
				/// </java-name>
    ...
    "/Future<TT;>;")]
				...
				/// <summary>
				/// <para>Submits a Runnable task for execution and returns a Future representing that task. The Future's <code>get</code> method will return the given result upon successful completion.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>a Future representing pending completion of the task </para>
				/// </returns>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Constructs an <code>ExecutionException</code> with no detail message. The cause is not initialized, and may subsequently be initialized by a call to initCause. </para>        
				/// </summary>
				...
				/// <summary>
				/// <para>Constructs an <code>ExecutionException</code> with the specified detail message and cause.</para><para></para>        
				/// </summary>
				[Dot42.DexImport("<init>", "(Ljava/lang/String;Ljava/lang/Throwable;)V", AccessFlags = 1)]
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>A java.util.Map providing additional atomic <code>putIfAbsent</code>, <code>remove</code>, and <code>replace</code> methods.</para><para>Memory consistency effects: As with other concurrent collections, actions in a thread prior to placing an object into a <c> ConcurrentMap </c> as a key or value  actions subsequent to the access or removal of that object from the <c> ConcurrentMap </c> in another thread.</para><para><para>1.5 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// </summary>
				/// <returns>
				/// <para>the previous value associated with the specified key, or <code>null</code> if there was no mapping for the key. (A <code>null</code> return can also indicate that the map previously associated <code>null</code> with the key, if the implementation supports null values.) </para>
				/// </returns>
				/// <java-name>
				/// putIfAbsent
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>An unbounded  BlockingQueue blocking queue} of <code>Delayed</code> elements, in which an element can only be taken when its delay has expired. The <b>head</b> of the queue is that <code>Delayed</code> element whose delay expired furthest in the past. If no delay has expired there is no head and <code>poll</code> will return <code>null</code>. Expiration occurs when an element's <code>getDelay(TimeUnit.NANOSECONDS)</code> method returns a value less than or equal to zero. Even though unexpired elements cannot be removed using <code>take</code> or <code>poll</code>, they are otherwise treated as normal elements. For example, the <code>size</code> method returns the count of both expired and unexpired elements. This queue does not permit null elements.</para><para>This class and its iterator implement all of the <b>optional</b> methods of the Collection and Iterator interfaces. The Iterator provided in method iterator() is <b>not</b> guaranteed to traverse the elements of the DelayQueue in any particular order.</para><para><para>1.5 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Creates a new <code>DelayQueue</code> that is initially empty. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>Creates a <code>DelayQueue</code> initially containing the elements of the given collection of Delayed instances.</para><para></para>        
				/// </summary>
				[Dot42.DexImport("<init>", "(Ljava/util/Collection;)V", AccessFlags = 1, Signature = "(Ljava/util/Collection<+TE;>;)V")]
				...
				/// <summary>
				/// <para>Retrieves and removes the head of this queue, or returns <code>null</code> if this queue has no elements with an expired delay.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the head of this queue, or <code>null</code> if this queue has no elements with an expired delay </para>
				/// </returns>
				/// <java-name>
				/// poll
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>the head of this queue, or <code>null</code> if the specified waiting time elapses before an element with an expired delay becomes available </para>
				/// </returns>
				/// <java-name>
				/// poll
				/// </java-name>
				...
				/// <summary>
				/// <para>Retrieves, but does not remove, the head of this queue, or returns <code>null</code> if this queue is empty. Unlike <code>poll</code>, if no expired elements are available in the queue, this method returns the element that will expire next, if one exists.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Always returns <code>Integer.MAX_VALUE</code> because a <code>DelayQueue</code> is not capacity constrained.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>Integer.MAX_VALUE</code> </para>
				/// </returns>
				/// <java-name>
				/// remainingCapacity
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns an array containing all of the elements in this queue; the runtime type of the returned array is that of the specified array. The returned array elements are in no particular order. If the queue fits in the specified array, it is returned therein. Otherwise, a new array is allocated with the runtime type of the specified array and the size of this queue.</para><para>If this queue fits in the specified array with room to spare (i.e., the array has more elements than this queue), the element in the array immediately following the end of the queue is set to <code>null</code>.</para><para>Like the toArray() method, this method acts as bridge between array-based and collection-based APIs. Further, this method allows precise control over the runtime type of the output array, and may, under certain circumstances, be used to save allocation costs.</para><para>The following code can be used to dump a delay queue into a newly allocated array of <code>Delayed</code>:</para><para><pre>  Delayed[] a = q.toArray(new Delayed[0]); 
				/// 
				///  </pre></para><para>Note that <code>toArray(new Object[0])</code> is identical in function to <code>toArray()</code>.</para><para></para>        
				/// </summary>
				/// <returns>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Constructs a <code>BrokenBarrierException</code> with no specified detail message. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>Constructs a <code>BrokenBarrierException</code> with the specified detail message.</para><para></para>        
				/// </summary>
				[Dot42.DexImport("<init>", "(Ljava/lang/String;)V", AccessFlags = 1)]
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Returns a shallow copy of this <code>ConcurrentSkipListMap</code> instance. (The keys and values themselves are not cloned.)</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this map contains a mapping for the specified key.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this map contains a mapping for the specified key </para>
				/// </returns>
				/// <java-name>
				/// containsKey
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>the previous value associated with the specified key, or <code>null</code> if there was no mapping for the key </para>
				/// </returns>
				/// <java-name>
				/// put
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this map maps one or more keys to the specified value. This operation requires time linear in the map size. Additionally, it is possible for the map to change during execution of this method, in which case the returned result may be inaccurate.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if a mapping to <code>value</code> exists; <code>false</code> otherwise </para>
				/// </returns>
				/// <java-name>
				/// containsValue
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns the number of key-value mappings in this map. If this map contains more than <code>Integer.MAX_VALUE</code> elements, it returns <code>Integer.MAX_VALUE</code>.</para><para>Beware that, unlike in most collections, this method is <b>NOT</b> a constant-time operation. Because of the asynchronous nature of these maps, determining the current number of elements requires traversing them all to count them. Additionally, it is possible for the size to change during execution of this method, in which case the returned result will be inaccurate. Thus, this method is typically not very useful in concurrent applications.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Returns <code>true</code> if this map contains no key-value mappings. </para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if this map contains no key-value mappings </para>
				/// </returns>
				...
				/// <summary>
				/// <para>Returns a Set view of the mappings contained in this map. The set's iterator returns the entries in ascending key order. The set is backed by the map, so changes to the map are reflected in the set, and vice-versa. The set supports element removal, which removes the corresponding mapping from the map, via the <code>Iterator.remove</code>, <code>Set.remove</code>, <code>removeAll</code>, <code>retainAll</code> and <code>clear</code> operations. It does not support the <code>add</code> or <code>addAll</code> operations.</para><para>The view's <code>iterator</code> is a "weakly consistent" iterator that will never throw ConcurrentModificationException, and guarantees to traverse elements as they existed upon construction of the iterator, and may (but is not guaranteed to) reflect any modifications subsequent to construction.</para><para>The <code>Map.Entry</code> elements returned by <code>iterator.next()</code> do <b>not</b> support the <code>setValue</code> operation.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Compares the specified object with this map for equality. Returns <code>true</code> if the given object is also a map and the two maps represent the same mappings. More formally, two maps <code>m1</code> and <code>m2</code> represent the same mappings if <code>m1.entrySet().equals(m2.entrySet())</code>. This operation may return misleading results if either map is concurrently modified during execution of this method.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para><code>true</code> if the specified object is equal to this map </para>
				/// </returns>
				/// <java-name>
				/// equals
				/// </java-name>
    ...
    "p;", AccessFlags = 1, Signature = "(TK;TK;)Ljava/util/concurrent/ConcurrentNavigableMap<TK;TV;>;")]
				...
				/// <summary>
				/// <para>Returns a key-value mapping associated with the greatest key strictly less than the given key, or <code>null</code> if there is no such key. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// lowerEntry
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns a key-value mapping associated with the greatest key less than or equal to the given key, or <code>null</code> if there is no such key. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// floorEntry
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns a key-value mapping associated with the least key greater than or equal to the given key, or <code>null</code> if there is no such entry. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// ceilingEntry
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns a key-value mapping associated with the least key strictly greater than the given key, or <code>null</code> if there is no such key. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// higherEntry
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns a key-value mapping associated with the least key in this map, or <code>null</code> if the map is empty. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method. </para>        
				/// </summary>
				/// <java-name>
				/// firstEntry
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns a key-value mapping associated with the greatest key in this map, or <code>null</code> if the map is empty. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method. </para>        
				/// </summary>
				/// <java-name>
				/// lastEntry
				/// </java-name>
				...
				/// <summary>
				/// <para>Removes and returns a key-value mapping associated with the least key in this map, or <code>null</code> if the map is empty. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method. </para>        
				/// </summary>
				/// <java-name>
				/// pollFirstEntry
				/// </java-name>
				...
				/// <summary>
				/// <para>Removes and returns a key-value mapping associated with the greatest key in this map, or <code>null</code> if the map is empty. The returned entry does <b>not</b> support the <code>Entry.setValue</code> method. </para>        
				/// </summary>
				/// <java-name>
				/// pollLastEntry
				/// </java-name>
				...
				[Dot42.DexImport("java/util/SortedMap", "headMap", "(Ljava/lang/Object;)Ljava/util/SortedMap;", AccessFlags = 1025, Signature = "(TK;)Ljava/util/SortedMap<TK;TV;>;")]
				global::Java.Util.ISortedMap<K, V> global::Java.Util.ISortedMap<K, V>.HeadMap(K endKey) /* TypeBuilder.AddAbstractInterfaceMethods */ 
				{
						return default(global::Java.Util.ISortedMap<K, V>);
				}

				[Dot42.DexImport("java/util/SortedMap", "subMap", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/util/SortedMap;", AccessFlags = 1025, Signature = "(TK;TK;)Ljava/util/SortedMap<TK;TV;>;")]
				global::Java.Util.ISortedMap<K, V> global::Java.Util.ISortedMap<K, V>.SubMap(K startKey, K endKey) /* TypeBuilder.AddAbstractInterfaceMethods */ 
				{
						return default(global::Java.Util.ISortedMap<K, V>);
				}

				[Dot42.DexImport("java/util/SortedMap", "tailMap", "(Ljava/lang/Object;)Ljava/util/SortedMap;", AccessFlags = 1025, Signature = "(TK;)Ljava/util/SortedMap<TK;TV;>;")]
				global::Java.Util.ISortedMap<K, V> global::Java.Util.ISortedMap<K, V>.TailMap(K startKey) /* TypeBuilder.AddAbstractInterfaceMethods */ 
				{
						return default(global::Java.Util.ISortedMap<K, V>);
				}
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>A Future that is Runnable. Successful execution of the <code>run</code> method causes completion of the <code>Future</code> and allows access to its results. <para>FutureTask </para><simplesectsep></simplesectsep><para>Executor </para><para>1.6 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>A ScheduledFuture that is Runnable. Successful execution of the <code>run</code> method causes completion of the <code>Future</code> and allows access to its results. <para>FutureTask </para><simplesectsep></simplesectsep><para>Executor </para><para>1.6 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		...
		///    ...
		///    </pre></para><para>However, the <code>Executor</code> interface does not strictly require that execution be asynchronous. In the simplest case, an executor can run the submitted task immediately in the caller's thread:</para><para><pre>    class DirectExecutor implements Executor {
		/// 
		...
		///    } 
		/// 
		///  </pre></para><para>Many <code>Executor</code> implementations impose some sort of limitation on how and when tasks are scheduled. The executor below serializes the submission of tasks to a second executor, illustrating a composite executor.</para><para><pre>    class SerialExecutor implements Executor {
		/// 
		...
		///    } 
		/// 
		///  </pre></para><para>The <code>Executor</code> implementations provided in this package implement ExecutorService, which is a more extensive interface. The ThreadPoolExecutor class provides an extensible thread pool implementation. The Executors class provides convenient factory methods for these Executors.</para><para>Memory consistency effects: Actions in a thread prior to submitting a <c> Runnable </c> object to an <c> Executor </c>  its execution begins, perhaps in another thread.</para><para><para>1.5 </para><para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Executes the given command at some time in the future. The command may execute in a new thread, in a pooled thread, or in the calling thread, at the discretion of the <code>Executor</code> implementation.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// execute
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
    ...
    "ecutionHandler;)V")]
				...
				/// </summary>
				/// <returns>
				/// <para>true if terminating but not yet terminated </para>
				/// </returns>
				/// <java-name>
				/// isTerminating
				/// </java-name>
				...
				/// <summary>
				/// <para>Sets the thread factory used to create new threads.</para><para><para>getThreadFactory </para></para>        
				/// </summary>
				/// <java-name>
				/// setThreadFactory
				/// </java-name>
				...
				/// <summary>
				/// <para>Returns the thread factory used to create new threads.</para><para><para>setThreadFactory </para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Sets a new handler for unexecutable tasks.</para><para><para>getRejectedExecutionHandler </para></para>        
				/// </summary>
				/// <java-name>
				/// setRejectedExecutionHandler
				/// </java-name>
				[Dot42.DexImport("setRejectedExecutionHandler", "(Ljava/util/concurrent/RejectedExecutionHandler;)V", AccessFlags = 1)]
				public virtual void SetRejectedExecutionHandler(global::Java.Util.Concurrent.IRejectedExecutionHandler handler) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				/// <para>Returns the current handler for unexecutable tasks.</para><para><para>setRejectedExecutionHandler </para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Sets the core number of threads. This overrides any value set in the constructor. If the new value is smaller than the current value, excess existing threads will be terminated when they next become idle. If larger, new threads will, if needed, be started to execute any queued tasks.</para><para><para>getCorePoolSize </para></para>        
				/// </summary>
				/// <java-name>
				/// setCorePoolSize
				/// </java-name>
				[Dot42.DexImport("setCorePoolSize", "(I)V", AccessFlags = 1)]
				public virtual void SetCorePoolSize(int corePoolSize) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Sets the maximum allowed number of threads. This overrides any value set in the constructor. If the new value is smaller than the current value, excess existing threads will be terminated when they next become idle.</para><para><para>getMaximumPoolSize </para></para>        
				/// </summary>
				/// <java-name>
				/// setMaximumPoolSize
				/// </java-name>
				[Dot42.DexImport("setMaximumPoolSize", "(I)V", AccessFlags = 1)]
				public virtual void SetMaximumPoolSize(int maximumPoolSize) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Returns the thread keep-alive time, which is the amount of time that threads in excess of the core pool size may remain idle before being terminated.</para><para><para>setKeepAliveTime </para></para>        
				/// </summary>
				/// <returns>
				...
				/// </summary>
				/// <returns>
				/// <para>true if the task was removed </para>
				/// </returns>
				/// <java-name>
				/// remove
				/// </java-name>
