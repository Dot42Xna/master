 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Returns the best possible localized form of the given skeleton for the given locale. A skeleton is similar to, and uses the same format characters as, a Unicode  pattern.</para><para>One difference is that order is irrelevant. For example, "MMMMd" will return "MMMM d" in the <c> en_US </c> locale, but "d. MMMM" in the <c> de_CH </c> locale.</para><para>Note also in that second example that the necessary punctuation for German was added. For the same input in <c> es_ES </c> , we'd have even more extra text: "d 'de' MMMM".</para><para>This method will automatically correct for grammatical necessity. Given the same "MMMMd" input, this method will return "d LLLL" in the <c> fa_IR </c> locale, where stand-alone months are necessary. Lengths are preserved where meaningful, so "Md" would give a different result to "MMMd", say, except in a locale such as <c> ja_JP </c> where there is only one length of month.</para><para>This method will only return patterns that are in CLDR, and is useful whenever you know what elements you want in your format string but don't want to make your code specific to any one locale.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>a string pattern suitable for use with java.text.SimpleDateFormat. </para>
				/// </returns>
				/// <java-name>
				/// getBestDateTimePattern
				/// </java-name>
				[Dot42.DexImport("getBestDateTimePattern", "(Ljava/util/Locale;Ljava/lang/String;)Ljava/lang/String;", AccessFlags = 9)]
				public static string GetBestDateTimePattern(global::Java.Util.Locale locale, string skeleton) /* MethodBuilder.Create */ 
				{
						return default(string);
				}
