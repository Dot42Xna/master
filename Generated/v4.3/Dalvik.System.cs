...
namespace Dalvik.System
{
 ...
 /* scope: __dot42__ */ 
		...
		public partial class DexClassLoader : global::Dalvik.System.BaseDexClassLoader
 /* scope: __dot42__ */ 
		{
				/// <summary>
		...
		/// <summary>
		/// <para>Base class for common functionality between various dex-based ClassLoader implementations. </para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Constructs an instance.</para><para></para>        
				/// </summary>
				...
				protected internal override global::System.Type FindClass(string name) /* MethodBuilder.Create */ 
				{
						return default(global::System.Type);
				}
				...
				protected internal override global::Java.Net.URL FindResource(string name) /* MethodBuilder.Create */ 
				{
				...
				protected internal override global::Java.Util.IEnumeration<global::Java.Net.URL> FindResources(string name) /* MethodBuilder.Create */ 
				{
				...
				public new virtual string FindLibrary(string name) /* MethodBuilder.Create */ 
				{
						return default(string);
				}

				/// <summary>
				/// <para>Returns package information for the given package. Unfortunately, instances of this class don't really have this information, and as a non-secure <c> ClassLoader </c> , it isn't even required to, according to the spec. Yet, we want to provide it, in order to make all those hopeful callers of <c> myClass.getPackage().getName() </c> happy. Thus we construct a <c> Package </c> object the first time it is being requested and fill most of the fields with dummy values. The <c>       Package </c> object is then put into the <c> ClassLoader </c> 's package cache, so we see the same one next time. We don't create <c> Package </c> objects for <c> null </c> arguments or for the default package.</para><para>There is a limited chance that we end up with multiple <c> Package </c> objects representing the same package: It can happen when when a package is scattered across different JAR files which were loaded by different <c> ClassLoader </c> instances. This is rather unlikely, and given that this whole thing is more or less a workaround, probably not worth the effort to address.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the package information for the class, or <c> null </c> if there is no package information available for it </para>
				/// </returns>
				/// <java-name>
				/// getPackage
				/// </java-name>
				[Dot42.DexImport("getPackage", "(Ljava/lang/String;)Ljava/lang/Package;", AccessFlags = 36)]
				protected internal override global::Java.Lang.Package GetPackage(string name) /* MethodBuilder.Create */ 
				{
