...
namespace Android.View.Inputmethod
{
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Gets the selected text, if any.</para><para>This method may fail if either the input connection has become invalid (such as its process crashing) or the client is taking too long to respond with the text (it is given a couple of seconds to return). In either case, null is returned.</para><para>This method must not cause any changes in the editor's state.</para><para>If GET_TEXT_WITH_STYLES is supplied as flags, the editor should return a android.text.SpannableString with all the spans set on the text.</para><para><b>IME authors:</b> please consider this will trigger an IPC round-trip that will take some time. Assume this method consumes a lot of time.</para><para><b>Editor authors:</b> please be careful of race conditions in implementing this call. An IME can make a change to the text or change the selection position and use this method right away; you need to make sure the returned value is consistent with the results of the latest edits.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the text that is currently selected, if any, or null if no text is selected. </para>
				/// </returns>
				/// <java-name>
				/// getSelectedText
				/// </java-name>
				[Dot42.DexImport("getSelectedText", "(I)Ljava/lang/CharSequence;", AccessFlags = 1025)]
				global::Java.Lang.ICharSequence GetSelectedText(int flags) /* MethodBuilder.Create */ ;
    ...
    "ctedText;", AccessFlags = 1025)]
				...
				/// <summary>
				/// <para>Mark a certain region of text as composing text. If there was a composing region, the characters are left as they were and the composing span removed, as if finishComposingText() has been called. The default style for composing text is used.</para><para>The passed indices are clipped to the contents bounds. If the resulting region is zero-sized, no region is marked and the effect is the same as that of calling finishComposingText(). The order of start and end is not important. In effect, the region from start to end and the region from end to start is the same. Editor authors, be ready to accept a start that is greater than end.</para><para>Since this does not change the contents of the text, editors should not call InputMethodManager#updateSelection(View, int, int, int, int) and IMEs should not receive android.inputmethodservice.InputMethodService#onUpdateSelection(int, int, int, int, int, int). </para><para>This has no impact on the cursor/selection position. It may result in the cursor being anywhere inside or outside the composing region, including cases where the selection and the composing region overlap partially or entirely.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>true on success, false if the input connection is no longer valid. </para>
				/// </returns>
				...
				bool SetComposingRegion(int start, int end) /* MethodBuilder.Create */ ;
				...
				/// <summary>
				/// <para>Commit a correction automatically performed on the raw user's input. A typical example would be to correct typos using a dictionary.</para><para>Calling this method will cause the editor to call android.inputmethodservice.InputMethodService#onUpdateSelection(int, int, int, int, int, int) on the current IME after the batch input is over. <b>Editor authors</b>, for this to happen you need to make the changes known to the input method by calling InputMethodManager#updateSelection(View, int, int, int, int), but be careful to wait until the batch edit is over if one is in progress.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>true on success, false if the input connection is no longer valid. </para>
				/// </returns>
		...
		/// <summary>
		/// <para>Information about a single text correction that an editor has reported to an input method. </para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				public CorrectionInfo(int offset, global::Java.Lang.ICharSequence oldText, global::Java.Lang.ICharSequence newText) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				/// <para>Return the offset position of this correction in the text. Both the getOldText() and getNewText() start at this offset. </para>        
				/// </summary>
				/// <java-name>
				/// getOffset
				/// </java-name>
				...
				/// <summary>
				/// <para>Return the text that has actually been typed by the user, and which has been corrected. </para>        
				/// </summary>
				/// <java-name>
				/// getOldText
				/// </java-name>
				...
				/// <summary>
				/// <para>Return the new text that corrects what was typed by the user. </para>        
				/// </summary>
				/// <java-name>
				/// getNewText
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
    ...
    "", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>Return the count of the subtypes of Input Method. </para>        
				/// </summary>
				/// <java-name>
				/// getSubtypeCount
				/// </java-name>
				...
				/// <summary>
				/// <para>Return the Input Method's subtype at the specified index.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// getSubtypeAt
				/// </java-name>
				[Dot42.DexImport("getSubtypeAt", "(I)Landroid/view/inputmethod/InputMethodSubtype;", AccessFlags = 1)]
				public global::Android.View.Inputmethod.InputMethodSubtype GetSubtypeAt(int index) /* MethodBuilder.Create */ 
				{
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Bits of IME_MASK_ACTION: Like IME_ACTION_NEXT, but for moving to the previous field. This will normally not be used to specify an action (since it precludes IME_ACTION_NEXT, but can be returned to the app if it sets IME_FLAG_NAVIGATE_PREVIOUS. </para>        
				/// </summary>
				/// <java-name>
				/// IME_ACTION_PREVIOUS
				/// </java-name>
				...
				/// <summary>
				/// <para>Flag of imeOptions: used to request that the IME never go into fullscreen mode. Applications need to be aware that the flag is not a guarantee, and not all IMEs will respect it. </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				/// <para>Flag of imeOptions: like IME_FLAG_NAVIGATE_NEXT, but specifies there is something interesting that a backward navigation can focus on. If the user selects the IME's facility to backward navigate, this will show up in the application as an IME_ACTION_PREVIOUS at InputConnection.performEditorAction(int). </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				/// <para>Flag of imeOptions: used to specify that there is something interesting that a forward navigation can focus on. This is like using IME_ACTION_NEXT, except allows the IME to be multiline (with an enter key) as well as provide forward navigation. Note that some IMEs may not be able to do this, especially when running on a small screen where there is little space. In that case it does not need to present a UI for this option. Like IME_ACTION_NEXT, if the user selects the IME's facility to forward navigate, this will show up in the application at InputConnection.performEditorAction(int). </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				/// <para>Ensure that the data in this EditorInfo is compatible with an application that was developed against the given target API version. This can impact the following input types: InputType#TYPE_TEXT_VARIATION_WEB_EMAIL_ADDRESS, InputType#TYPE_TEXT_VARIATION_WEB_PASSWORD, InputType#TYPE_NUMBER_VARIATION_NORMAL, InputType#TYPE_NUMBER_VARIATION_PASSWORD.</para><para>This is called by the framework for input method implementations; you should not generally need to call it yourself.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// makeCompatible
				/// </java-name>
				[Dot42.DexImport("makeCompatible", "(I)V", AccessFlags = 17)]
				public void MakeCompatible(int targetSdkVersion) /* MethodBuilder.Create */ 
				{
				}
		...
		/// <summary>
		/// <para>This class is used to specify meta information of a subtype contained in an input method editor (IME). Subtype can describe locale (e.g. en_US, fr_FR...) and mode (e.g. voice, keyboard...), and is used for IME switch and settings. The input method subtype allows the system to bring up the specified subtype of the designated IME directly.</para><para>It should be defined in an XML resource file of the input method with the <code>&lt;subtype&gt;</code> element. For more information, see the guide to .</para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Constructor with no subtype ID specified. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "(IILjava/lang/String;Ljava/lang/String;Ljava/lang/String;ZZ)V", AccessFlags = 1)]
				public InputMethodSubtype(int nameId, int iconId, string locale, string mode, string extraValue, bool isAuxiliary, bool overridesImplicitlyEnabledSubtype) /* MethodBuilder.Create */ 
				{
				}
				...
				/// </summary>
				[Dot42.DexImport("<init>", "(IILjava/lang/String;Ljava/lang/String;Ljava/lang/String;ZZI)V", AccessFlags = 1)]
				public InputMethodSubtype(int nameId, int iconId, string locale, string mode, string extraValue, bool isAuxiliary, bool overridesImplicitlyEnabledSubtype, int id) /* MethodBuilder.Create */ 
				{
				}
				...
				/// </summary>
				/// <returns>
				/// <para>Resource ID of the subtype name string. </para>
				/// </returns>
				/// <java-name>
				/// getNameResId
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>Resource ID of the subtype icon drawable. </para>
				/// </returns>
				/// <java-name>
				/// getIconResId
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>The locale of the subtype. This method returns the "locale" string parameter passed to the constructor. </para>
				/// </returns>
				/// <java-name>
				/// getLocale
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>The mode of the subtype. </para>
				/// </returns>
				/// <java-name>
				/// getMode
				/// </java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>true when this subtype will be enabled by default if no other subtypes in the IME are enabled explicitly, false otherwise. Note that a subtype with this method returning true will not be shown in the list of subtypes in each IME's subtype enabler. Having an "automatic" subtype is an example use of this flag. </para>
				/// </returns>
				/// <java-name>
				...
				/// isAsciiCapable
				/// </java-name>
				[Dot42.DexImport("isAsciiCapable", "()Z", AccessFlags = 1)]
				public bool IsAsciiCapable() /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
    ...
    "Ljava/lang/CharSequence;", AccessFlags = 1)]
				...
				/// </java-name>
				[Dot42.DexImport("android/view/inputmethod/InputMethodSubtype$InputMethodSubtypeBuilder", AccessFlags = 9)]
				public partial class InputMethodSubtypeBuilder
 /* scope: __dot42__ */ 
				{
						[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
						public InputMethodSubtypeBuilder() /* MethodBuilder.Create */ 
						{
						}
						...
						/// setIsAuxiliary
						/// </java-name>
						[Dot42.DexImport("setIsAuxiliary", "(Z)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubtypeBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetIsAuxiliary(bool boolean) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}

						/// <java-name>
						/// setOverridesImplicitlyEnabledSubtype
						/// </java-name>
						[Dot42.DexImport("setOverridesImplicitlyEnabledSubtype", "(Z)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubtypeBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetOverridesImplicitlyEnabledSubtype(bool boolean) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// setIsAsciiCapable
						/// </java-name>
						[Dot42.DexImport("setIsAsciiCapable", "(Z)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubtypeBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetIsAsciiCapable(bool boolean) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// setSubtypeIconResId
						/// </java-name>
						[Dot42.DexImport("setSubtypeIconResId", "(I)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubtypeBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetSubtypeIconResId(int int32) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// setSubtypeNameResId
						/// </java-name>
						[Dot42.DexImport("setSubtypeNameResId", "(I)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubtypeBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetSubtypeNameResId(int int32) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// setSubtypeId
						/// </java-name>
						[Dot42.DexImport("setSubtypeId", "(I)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubtypeBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetSubtypeId(int int32) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// setSubtypeLocale
						/// </java-name>
						[Dot42.DexImport("setSubtypeLocale", "(Ljava/lang/String;)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubty" +
    "peBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetSubtypeLocale(string @string) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// setSubtypeMode
						/// </java-name>
						[Dot42.DexImport("setSubtypeMode", "(Ljava/lang/String;)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubty" +
    "peBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetSubtypeMode(string @string) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// </java-name>
						[Dot42.DexImport("setSubtypeExtraValue", "(Ljava/lang/String;)Landroid/view/inputmethod/InputMethodSubtype$InputMethodSubty" +
    "peBuilder;", AccessFlags = 1)]
						public virtual InputMethodSubtypeBuilder SetSubtypeExtraValue(string @string) /* MethodBuilder.Create */ 
						{
								return default(InputMethodSubtypeBuilder);
						}
						...
						/// build
						/// </java-name>
						[Dot42.DexImport("build", "()Landroid/view/inputmethod/InputMethodSubtype;", AccessFlags = 1)]
						public virtual global::Android.View.Inputmethod.InputMethodSubtype Build() /* MethodBuilder.Create */ 
						{
								return default(global::Android.View.Inputmethod.InputMethodSubtype);
						}
 ...
 /* scope: __dot42__ */ 
		{
				...
				void ViewClicked(bool focusChanged) /* MethodBuilder.Create */ ;
    ...
    "lback;)V", AccessFlags = 1025)]
				...
				/// <summary>
				/// <para>This method is called when there is a generic motion event.</para><para>If the input method wants to handle this event, return true, otherwise return false and the caller (i.e. the application) will handle the event.</para><para><para>android.view.MotionEvent </para></para>        
				/// </summary>
				/// <returns>
				/// <para>Whether the input method wants to handle this event.</para>
				/// </returns>
    ...
    "lback;)V", AccessFlags = 1025)]
				void DispatchGenericMotionEvent(int seq, global::Android.View.MotionEvent @event, global::Android.View.Inputmethod.IInputMethodSession_IEventCallback callback) /* MethodBuilder.Create */ ;
 ...
 /* scope: __dot42__ */ 
    ...
    "ethod/InputMethodSubtype;>;")]
				public global::Java.Util.IList<global::Android.View.Inputmethod.InputMethodSubtype> GetEnabledInputMethodSubtypeList(global::Android.View.Inputmethod.InputMethodInfo imi, bool allowsImplicitlySelectedSubtypes) /* MethodBuilder.Create */ 
				{
				...
				/// <summary>
				/// <para>Force switch to a new input method and subtype. This can only be called from an application or a service which has a token of the currently active input method. </para>        
				/// </summary>
				/// <java-name>
    ...
    "pe;)V", AccessFlags = 1)]
				public void SetInputMethodAndSubtype(global::Android.Os.IBinder token, string id, global::Android.View.Inputmethod.InputMethodSubtype subtype) /* MethodBuilder.Create */ 
				{
				}
				...
				public void ShowInputMethodAndSubtypeEnabler(string imiId) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				/// <para>Returns the current input method subtype. This subtype is one of the subtypes in the current input method. This method returns null when the current input method doesn't have any input method subtype. </para>        
				/// </summary>
				/// <java-name>
				...
				/// </summary>
				/// <returns>
				/// <para>true if the current subtype was successfully switched. When the specified subtype is null, this method returns false. </para>
				/// </returns>
				/// <java-name>
				...
				public bool SetCurrentInputMethodSubtype(global::Android.View.Inputmethod.InputMethodSubtype subtype) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
    ...
    "id/view/inputmethod/InputMethodSubtype;>;>;")]
				...
				/// <summary>
				/// <para>Force switch to the last used input method and subtype. If the last input method didn't have any subtypes, the framework will simply switch to the last input method with no subtype specified. </para>        
				/// </summary>
				/// <returns>
				/// <para>true if the current input method and subtype was successfully switched to the last used input method and subtype. </para>
				/// </returns>
				/// <java-name>
				/// switchToLastInputMethod
				/// </java-name>
				[Dot42.DexImport("switchToLastInputMethod", "(Landroid/os/IBinder;)Z", AccessFlags = 1)]
				public bool SwitchToLastInputMethod(global::Android.Os.IBinder imeToken) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}

				/// <summary>
				/// <para>Force switch to the next input method and subtype. If there is no IME enabled except current IME and subtype, do nothing. </para>        
				/// </summary>
				/// <returns>
				/// <para>true if the current input method and subtype was successfully switched to the next input method and subtype. </para>
				/// </returns>
				/// <java-name>
				/// switchToNextInputMethod
				/// </java-name>
				[Dot42.DexImport("switchToNextInputMethod", "(Landroid/os/IBinder;Z)Z", AccessFlags = 1)]
				public bool SwitchToNextInputMethod(global::Android.Os.IBinder imeToken, bool onlyCurrentIme) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
				...
				/// </java-name>
				[Dot42.DexImport("shouldOfferSwitchingToNextInputMethod", "(Landroid/os/IBinder;)Z", AccessFlags = 1)]
				public bool ShouldOfferSwitchingToNextInputMethod(global::Android.Os.IBinder binder) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}

				/// <summary>
				/// <para>Set additional input method subtypes. Only a process which shares the same uid with the IME can add additional input method subtypes to the IME. Please note that a subtype's status is stored in the system. For example, enabled subtypes are remembered by the framework even after they are removed by using this method. If you re-add the same subtypes again, they will just get enabled. If you want to avoid such conflicts, for instance, you may want to create a "different" new subtype even with the same locale and mode, by changing its extra value. The different subtype won't get affected by the stored past status. (You may want to take a look at InputMethodSubtype#hashCode() to refer to the current implementation.) </para>        
				/// </summary>
				/// <java-name>
				...
				public void SetAdditionalInputMethodSubtypes(string imiId, global::Android.View.Inputmethod.InputMethodSubtype[] subtypes) /* MethodBuilder.Create */ 
				{
				}
 ...
 /* scope: __dot42__ */ 
    ...
    "V", AccessFlags = 1025)]
				...
				/// <summary>
				/// <para>Notify that the input method subtype is being changed in the same input method. </para>        
				/// </summary>
				/// <java-name>
				...
				void ChangeInputMethodSubtype(global::Android.View.Inputmethod.InputMethodSubtype subtype) /* MethodBuilder.Create */ ;
 ...
 /* scope: __dot42__ */ 
    ...
    "ctedText;", AccessFlags = 1)]
				...
				/// <summary>
				/// <para>The default implementation returns the text currently selected, or null if none is selected. </para>        
				/// </summary>
				/// <java-name>
				/// getSelectedText
				/// </java-name>
				[Dot42.DexImport("getSelectedText", "(I)Ljava/lang/CharSequence;", AccessFlags = 1)]
				public virtual global::Java.Lang.ICharSequence GetSelectedText(int flags) /* MethodBuilder.Create */ 
				{
						return default(global::Java.Lang.ICharSequence);
				}
				...
				public virtual bool SetComposingRegion(int start, int end) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
