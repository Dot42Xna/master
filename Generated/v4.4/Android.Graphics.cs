...
//
// Original filename: Android.Graphics.cs
//
...
namespace Android.Graphics
{
 ...
 /* scope: __dot42__ */ 
		{
				...
				public SurfaceTexture(int texName, bool allowSynchronousMode) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Set the default size of the image buffers. The image producer may override the buffer size, in which case the producer-set buffer size will be used, not the default size set by this method. Both video and camera based image producers do override the size. This method may be used to set the image size when producing images with android.graphics.Canvas (via android.view.Surface#lockCanvas), or OpenGL ES (via an EGLSurface).</para><para>The new default buffer size will take effect the next time the image producer requests a buffer to fill. For android.graphics.Canvas this will be the next time android.view.Surface#lockCanvas is called. For OpenGL ES, the EGLSurface should be destroyed (via eglDestroySurface), made not-current (via eglMakeCurrent), and then recreated (via eglCreateWindowSurface) to ensure that the new default size has taken effect.</para><para>The width and height parameters must be no greater than the minimum of GL_MAX_VIEWPORT_DIMS and GL_MAX_TEXTURE_SIZE (see glGetIntegerv). An error due to invalid dimensions might not be reported until updateTexImage() is called. </para>        
				/// </summary>
				/// <java-name>
				/// setDefaultBufferSize
				/// </java-name>
				[Dot42.DexImport("setDefaultBufferSize", "(II)V", AccessFlags = 1)]
				public virtual void SetDefaultBufferSize(int width, int height) /* MethodBuilder.Create */ 
				{
				}
				...
				/// releaseTexImage
				/// </java-name>
				[Dot42.DexImport("releaseTexImage", "()V", AccessFlags = 1)]
				public virtual void ReleaseTexImage() /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				/// <para>Detach the SurfaceTexture from the OpenGL ES context that owns the OpenGL ES texture object. This call must be made with the OpenGL ES context current on the calling thread. The OpenGL ES texture object will be deleted as a result of this call. After calling this method all calls to updateTexImage will throw an java.lang.IllegalStateException until a successful call to attachToGLContext is made.</para><para>This can be used to access the SurfaceTexture image contents from multiple OpenGL ES contexts. Note, however, that the image contents are only accessible from one OpenGL ES context at a time. </para>        
				/// </summary>
				/// <java-name>
				/// detachFromGLContext
				/// </java-name>
				[Dot42.DexImport("detachFromGLContext", "()V", AccessFlags = 1)]
				public virtual void DetachFromGLContext() /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				/// <para>Attach the SurfaceTexture to the OpenGL ES context that is current on the calling thread. A new OpenGL ES texture object is created and populated with the SurfaceTexture image frame that was current at the time of the last call to detachFromGLContext. This new texture is bound to the GL_TEXTURE_EXTERNAL_OES texture target.</para><para>This can be used to access the SurfaceTexture image contents from multiple OpenGL ES contexts. Note, however, that the image contents are only accessible from one OpenGL ES context at a time.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// attachToGLContext
				/// </java-name>
				[Dot42.DexImport("attachToGLContext", "(I)V", AccessFlags = 1)]
				public virtual void AttachToGLContext(int texName) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>release() frees all the buffers and puts the SurfaceTexture into the 'abandoned' state. Once put in this state the SurfaceTexture can never leave it. When in the 'abandoned' state, all methods of the IGraphicBufferProducer interface will fail with the NO_INIT error.</para><para>Note that while calling this method causes all the buffers to be freed from the perspective of the the SurfaceTexture, if there are additional references on the buffers (e.g. if a buffer is referenced by a client or by OpenGL ES as a texture) then those buffer will remain allocated.</para><para>Always call this method when you are done with SurfaceTexture. Failing to do so may delay resource deallocation for a significant amount of time. </para>        
				/// </summary>
				/// <java-name>
				/// release
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Applies a rotation transform around the X axis.</para><para><para>rotateY(float) </para><simplesectsep></simplesectsep><para>rotateZ(float) </para><simplesectsep></simplesectsep><para>rotate(float, float, float) </para></para>        
				/// </summary>
				/// <java-name>
				/// rotateX
				/// </java-name>
				...
				/// <summary>
				/// <para>Applies a rotation transform around the Y axis.</para><para><para>rotateX(float) </para><simplesectsep></simplesectsep><para>rotateZ(float) </para><simplesectsep></simplesectsep><para>rotate(float, float, float) </para></para>        
				/// </summary>
				/// <java-name>
				/// rotateY
				/// </java-name>
				...
				/// <summary>
				/// <para>Applies a rotation transform around the Z axis.</para><para><para>rotateX(float) </para><simplesectsep></simplesectsep><para>rotateY(float) </para><simplesectsep></simplesectsep><para>rotate(float, float, float) </para></para>        
				/// </summary>
				/// <java-name>
				/// rotateZ
				/// </java-name>
				...
				/// <summary>
				/// <para>Applies a rotation transform around all three axis.</para><para><para>rotateX(float) </para><simplesectsep></simplesectsep><para>rotateY(float) </para><simplesectsep></simplesectsep><para>rotateZ(float) </para></para>        
				/// </summary>
				/// <java-name>
				/// rotate
				/// </java-name>
				...
				/// <summary>
				/// <para>Gets the x location of the camera.</para><para><para>setLocation(float, float, float) </para></para>        
				/// </summary>
				/// <java-name>
				/// getLocationX
				/// </java-name>
				[Dot42.DexImport("getLocationX", "()F", AccessFlags = 257)]
				public virtual float GetLocationX() /* MethodBuilder.Create */ 
				{
						return default(float);
				}

				/// <summary>
				/// <para>Gets the y location of the camera.</para><para><para>setLocation(float, float, float) </para></para>        
				/// </summary>
				/// <java-name>
				/// getLocationY
				/// </java-name>
				[Dot42.DexImport("getLocationY", "()F", AccessFlags = 257)]
				public virtual float GetLocationY() /* MethodBuilder.Create */ 
				{
						return default(float);
				}

				/// <summary>
				/// <para>Gets the z location of the camera.</para><para><para>setLocation(float, float, float) </para></para>        
				/// </summary>
				/// <java-name>
				/// getLocationZ
				/// </java-name>
				[Dot42.DexImport("getLocationZ", "()F", AccessFlags = 257)]
				public virtual float GetLocationZ() /* MethodBuilder.Create */ 
				{
						return default(float);
				}
 ...
 /* scope: __dot42__ */ 
    ...
    "s/Bitmap;", AccessFlags = 1)]
		...
		/// <summary>
		/// <para>The Matrix class holds a 3x3 matrix for transforming coordinates. Matrix does not have a constructor, so it must be explicitly initialized using either reset() - to construct an identity matrix, or one of the set..() functions (e.g. setTranslate, setRotate, etc.). </para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				[Dot42.DexImport("<init>", "(Landroid/graphics/Bitmap;[B)V", AccessFlags = 1)]
				public NinePatch(global::Android.Graphics.Bitmap bitmap, sbyte[] sByte) /* MethodBuilder.Create */ 
				{
				}

				[Dot42.DexImport("<init>", "(Landroid/graphics/Bitmap;[B)V", AccessFlags = 1, IgnoreFromJava = true)]
				public NinePatch(global::Android.Graphics.Bitmap bitmap, byte[] @byte) /* MethodBuilder.Create */ 
				{
				}
				...
				/// </java-name>
				[Dot42.DexImport("finalize", "()V", AccessFlags = 4)]
				extern ~NinePatch() /* MethodBuilder.Create */ ;
				...
				public virtual global::Android.Graphics.Bitmap GetBitmap() /* MethodBuilder.Create */ 
				{
						return default(global::Android.Graphics.Bitmap);
				}

				/// <summary>
				/// <para>Draw a bitmap of nine patches.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// draw
				/// </java-name>
				...
				public global::Android.Graphics.Region GetTransparentRegion(global::Android.Graphics.Rect location) /* MethodBuilder.Create */ 
				{
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		/// <para>A subclass of shader that returns the coposition of two other shaders, combined by an android.graphics.Xfermode subclass. </para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// getPixelFormatInfo
				/// </java-name>
				[Dot42.DexImport("getPixelFormatInfo", "(ILandroid/graphics/PixelFormat;)V", AccessFlags = 9)]
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>Indicates that the bitmap was created for an unknown pixel density.</para><para><para>Bitmap::getDensity() </para><simplesectsep></simplesectsep><para>Bitmap::setDensity(int) </para></para>        
				/// </summary>
				/// <java-name>
				/// DENSITY_NONE
				/// </java-name>
				...
				/// <summary>
				/// <para>Specifies the density for this bitmap. When the bitmap is drawn to a Canvas that also has a density, it will be scaled appropriately.</para><para><para>getDensity() </para><simplesectsep></simplesectsep><para>android.util.DisplayMetrics::DENSITY_DEFAULT </para><simplesectsep></simplesectsep><para>android.util.DisplayMetrics::densityDpi </para><simplesectsep></simplesectsep><para>DENSITY_NONE </para></para>        
				/// </summary>
				/// <java-name>
				/// setDensity
				/// </java-name>
				[Dot42.DexImport("setDensity", "(I)V", AccessFlags = 1)]
				public void SetDensity(int density) /* MethodBuilder.Create */ 
				{
				}
				...
				/// reconfigure
				/// </java-name>
				[Dot42.DexImport("reconfigure", "(IILandroid/graphics/Bitmap$Config;)V", AccessFlags = 1)]
				public void Reconfigure(int int32, int int321, global::Android.Graphics.Bitmap.Config config) /* MethodBuilder.Create */ 
				{
				}
				...
				public void SetWidth(int int32) /* MethodBuilder.Create */ 
				{
				}
				...
				public void SetHeight(int int32) /* MethodBuilder.Create */ 
				{
				}
				...
				/// setConfig
				/// </java-name>
				[Dot42.DexImport("setConfig", "(Landroid/graphics/Bitmap$Config;)V", AccessFlags = 1)]
				public void SetConfig(global::Android.Graphics.Bitmap.Config config) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Creates a new bitmap, scaled from an existing bitmap, when possible. If the specified width and height are the same as the current width and height of the source btimap, the source bitmap is returned and now new bitmap is created.</para><para></para>        
				/// </summary>
				/// <returns>
    ...
    ";", AccessFlags = 9)]
				...
				/// createBitmap
				/// </java-name>
				[Dot42.DexImport("createBitmap", "(Landroid/util/DisplayMetrics;IILandroid/graphics/Bitmap$Config;)Landroid/graphic" +
    "s/Bitmap;", AccessFlags = 9)]
				public static global::Android.Graphics.Bitmap CreateBitmap(global::Android.Util.DisplayMetrics displayMetrics, int int32, int int321, global::Android.Graphics.Bitmap.Config config) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Graphics.Bitmap);
				}
				...
				/// createBitmap
				/// </java-name>
				[Dot42.DexImport("createBitmap", "(Landroid/util/DisplayMetrics;[IIIIILandroid/graphics/Bitmap$Config;)Landroid/gra" +
    "phics/Bitmap;", AccessFlags = 9)]
				public static global::Android.Graphics.Bitmap CreateBitmap(global::Android.Util.DisplayMetrics displayMetrics, int[] int32, int int321, int int322, int int323, int int324, global::Android.Graphics.Bitmap.Config config) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Graphics.Bitmap);
				}
				...
				/// createBitmap
				/// </java-name>
				[Dot42.DexImport("createBitmap", "(Landroid/util/DisplayMetrics;[IIILandroid/graphics/Bitmap$Config;)Landroid/graph" +
    "ics/Bitmap;", AccessFlags = 9)]
				public static global::Android.Graphics.Bitmap CreateBitmap(global::Android.Util.DisplayMetrics source, int[] x, int y, int width, global::Android.Graphics.Bitmap.Config height) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Graphics.Bitmap);
				}
				...
				/// <summary>
				/// <para>Indicates whether pixels stored in this bitmaps are stored pre-multiplied. When a pixel is pre-multiplied, the RGB components have been multiplied by the alpha component. For instance, if the original color is a 50% translucent red <code>(128, 255, 0, 0)</code>, the pre-multiplied form is <code>(128, 128, 0, 0)</code>.</para><para>This method always returns false if getConfig() is Bitmap.Config#RGB_565.</para><para>This method only returns true if hasAlpha() returns true. A bitmap with no alpha channel can be used both as a pre-multiplied and as a non pre-multiplied bitmap.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>true if the underlying pixels have been pre-multiplied, false otherwise </para>
				/// </returns>
				/// <java-name>
				/// isPremultiplied
				/// </java-name>
				[Dot42.DexImport("isPremultiplied", "()Z", AccessFlags = 17)]
				public bool IsPremultiplied() /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
				...
				/// setPremultiplied
				/// </java-name>
				[Dot42.DexImport("setPremultiplied", "(Z)V", AccessFlags = 17)]
				public void SetPremultiplied(bool boolean) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Return the number of bytes between rows in the bitmap's pixels. Note that this refers to the pixels as stored natively by the bitmap. If you call getPixels() or setPixels(), then the pixels are uniformly treated as 32bit values, packed according to the Color class.</para><para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Returns the number of bytes used to store this bitmap's pixels. </para>        
				/// </summary>
				/// <java-name>
				/// getByteCount
				/// </java-name>
				...
				/// </java-name>
				[Dot42.DexImport("getAllocationByteCount", "()I", AccessFlags = 17)]
				public int GetAllocationByteCount() /* MethodBuilder.Create */ 
				{
						return default(int);
				}
				...
				/// <summary>
				/// <para>Indicates whether the renderer responsible for drawing this bitmap should attempt to use mipmaps when this bitmap is drawn scaled down.</para><para>If you know that you are going to draw this bitmap at less than 50% of its original size, you may be able to obtain a higher quality</para><para>This property is only a suggestion that can be ignored by the renderer. It is not guaranteed to have any effect.</para><para><para>setHasMipMap(boolean) </para></para>        
				/// </summary>
				/// <returns>
				/// <para>true if the renderer should attempt to use mipmaps, false otherwise</para>
				/// </returns>
				/// <java-name>
				/// hasMipMap
				/// </java-name>
				[Dot42.DexImport("hasMipMap", "()Z", AccessFlags = 17)]
				public bool HasMipMap() /* MethodBuilder.Create */ 
				{
						return default(bool);
				}

				/// <summary>
				/// <para>Set a hint for the renderer responsible for drawing this bitmap indicating that it should attempt to use mipmaps when this bitmap is drawn scaled down.</para><para>If you know that you are going to draw this bitmap at less than 50% of its original size, you may be able to obtain a higher quality by turning this property on.</para><para>Note that if the renderer respects this hint it might have to allocate extra memory to hold the mipmap levels for this bitmap.</para><para>This property is only a suggestion that can be ignored by the renderer. It is not guaranteed to have any effect.</para><para><para>hasMipMap() </para></para>        
				/// </summary>
				/// <java-name>
				/// setHasMipMap
				/// </java-name>
				[Dot42.DexImport("setHasMipMap", "(Z)V", AccessFlags = 17)]
				public void SetHasMipMap(bool hasMipMap) /* MethodBuilder.Create */ 
				{
				}
				...
				{
				[Dot42.DexImport("getAllocationByteCount", "()I", AccessFlags = 17)]
						get{ return GetAllocationByteCount(); }
				}
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				/// <para>bit mask for the flag enabling antialiasing </para>        
				/// </summary>
				/// <java-name>
				/// ANTI_ALIAS_FLAG
				/// </java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling bitmap filtering </para>        
				/// </summary>
				/// <java-name>
				/// FILTER_BITMAP_FLAG
				/// </java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling dithering </para>        
				/// </summary>
				/// <java-name>
				/// DITHER_FLAG
				/// </java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling underline text </para>        
				/// </summary>
				/// <java-name>
				/// UNDERLINE_TEXT_FLAG
				/// </java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling strike-thru text </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling fake-bold text </para>        
				/// </summary>
				/// <java-name>
				/// FAKE_BOLD_TEXT_FLAG
				/// </java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling linear-text (no caching) </para>        
				/// </summary>
				/// <java-name>
				/// LINEAR_TEXT_FLAG
				/// </java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling subpixel-text </para>        
				/// </summary>
				/// <java-name>
				/// SUBPIXEL_TEXT_FLAG
				/// </java-name>
				...
				/// <summary>
				/// <para>bit mask for the flag enabling device kerning for text </para>        
				/// </summary>
				/// <java-name>
				/// DEV_KERN_TEXT_FLAG
				/// </java-name>
				...
				/// </java-name>
				[Dot42.DexImport("EMBEDDED_BITMAP_TEXT_FLAG", "I", AccessFlags = 25)]
				public const int EMBEDDED_BITMAP_TEXT_FLAG = 1024;
				/// <summary>
				/// <para>Option for setHinting: disable hinting. </para>        
				/// </summary>
				/// <java-name>
				/// HINTING_OFF
				/// </java-name>
				...
				/// <summary>
				/// <para>Option for setHinting: enable hinting. </para>        
				/// </summary>
				/// <java-name>
				/// HINTING_ON
				/// </java-name>
				...
				/// <summary>
				/// <para>Set the paint's flags. Use the Flag enum to specific flag values.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setFlags
				/// </java-name>
				[Dot42.DexImport("setFlags", "(I)V", AccessFlags = 257)]
				public virtual void SetFlags(int flags) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				/// <para>Return the paint's hinting mode. Returns either HINTING_OFF or HINTING_ON. </para>        
				/// </summary>
				/// <java-name>
				/// getHinting
				/// </java-name>
				...
				/// <summary>
				/// <para>Set the paint's hinting mode. May be either HINTING_OFF or HINTING_ON. </para>        
				/// </summary>
				/// <java-name>
				/// setHinting
				/// </java-name>
				[Dot42.DexImport("setHinting", "(I)V", AccessFlags = 257)]
				public virtual void SetHinting(int mode) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the ANTI_ALIAS_FLAG bit AntiAliasing smooths out the edges of what is being drawn, but is has no impact on the interior of the shape. See setDither() and setFilterBitmap() to affect how colors are treated.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setAntiAlias
				/// </java-name>
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the DITHER_FLAG bit Dithering affects how colors that are higher precision than the device are down-sampled. No dithering is generally faster, but higher precision colors are just truncated down (e.g. 8888 -&gt; 565). Dithering tries to distribute the error inherent in this process, to reduce the visual artifacts.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setDither
				/// </java-name>
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the LINEAR_TEXT_FLAG bit</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setLinearText
				/// </java-name>
				[Dot42.DexImport("setLinearText", "(Z)V", AccessFlags = 257)]
				public virtual void SetLinearText(bool linearText) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the SUBPIXEL_TEXT_FLAG bit</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setSubpixelText
				/// </java-name>
				[Dot42.DexImport("setSubpixelText", "(Z)V", AccessFlags = 257)]
				public virtual void SetSubpixelText(bool subpixelText) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the UNDERLINE_TEXT_FLAG bit</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setUnderlineText
				/// </java-name>
				[Dot42.DexImport("setUnderlineText", "(Z)V", AccessFlags = 257)]
				public virtual void SetUnderlineText(bool underlineText) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the STRIKE_THRU_TEXT_FLAG bit</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setStrikeThruText
				/// </java-name>
				[Dot42.DexImport("setStrikeThruText", "(Z)V", AccessFlags = 257)]
				public virtual void SetStrikeThruText(bool strikeThruText) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the FAKE_BOLD_TEXT_FLAG bit</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setFakeBoldText
				/// </java-name>
				[Dot42.DexImport("setFakeBoldText", "(Z)V", AccessFlags = 257)]
				public virtual void SetFakeBoldText(bool fakeBoldText) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Helper for setFlags(), setting or clearing the FILTER_BITMAP_FLAG bit. Filtering affects the sampling of bitmaps when they are transformed. Filtering does not affect how the colors in the bitmap are converted into device pixels. That is dependent on dithering and xfermodes.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setFilterBitmap
				/// </java-name>
				...
				/// <summary>
				/// <para>Set the paint's color. Note that the color is an int containing alpha as well as r,g,b. This 32bit value is not premultiplied, meaning that its alpha can be any value, regardless of the values of r,g,b. See the Color class for more details.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setColor
				/// </java-name>
				...
				/// <summary>
				/// <para>Helper to setColor(), that only assigns the color's alpha value, leaving its r,g,b values unchanged. Results are undefined if the alpha value is outside of the range [0..255]</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setAlpha
				/// </java-name>
				[Dot42.DexImport("setAlpha", "(I)V", AccessFlags = 257)]
				public virtual void SetAlpha(int a) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Set the width for stroking. Pass 0 to stroke in hairline mode. Hairlines always draws a single pixel independent of the canva's matrix.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setStrokeWidth
				/// </java-name>
				[Dot42.DexImport("setStrokeWidth", "(F)V", AccessFlags = 257)]
				public virtual void SetStrokeWidth(float width) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Set the paint's stroke miter value. This is used to control the behavior of miter joins when the joins angle is sharp. This value must be &gt;= 0.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setStrokeMiter
				/// </java-name>
				[Dot42.DexImport("setStrokeMiter", "(F)V", AccessFlags = 257)]
				public virtual void SetStrokeMiter(float miter) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Set the paint's Cap.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setStrokeCap
				/// </java-name>
				...
				/// <summary>
				/// <para>Set the paint's Join.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setStrokeJoin
				/// </java-name>
				...
				/// <summary>
				/// <para>Set the paint's text alignment. This controls how the text is positioned relative to its origin. LEFT align means that all of the text will be drawn to the right of its origin (i.e. the origin specifieds the LEFT edge of the text) and so on.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setTextAlign
				/// </java-name>
				...
				/// <summary>
				/// <para>Get the text Locale.</para><para></para>        
				/// </summary>
				/// <returns>
				/// <para>the paint's Locale used for drawing text, never null. </para>
				/// </returns>
				/// <java-name>
				/// getTextLocale
				/// </java-name>
				[Dot42.DexImport("getTextLocale", "()Ljava/util/Locale;", AccessFlags = 1)]
				public virtual global::Java.Util.Locale GetTextLocale() /* MethodBuilder.Create */ 
				{
						return default(global::Java.Util.Locale);
				}

				/// <summary>
				/// <para>Set the text locale.</para><para>The text locale affects how the text is drawn for some languages.</para><para>For example, if the locale is Locale#CHINESE or Locale#CHINA, then the text renderer will prefer to draw text using a Chinese font. Likewise, if the locale is Locale#JAPANESE or Locale#JAPAN, then the text renderer will prefer to draw text using a Japanese font.</para><para>This distinction is important because Chinese and Japanese text both use many of the same Unicode code points but their appearance is subtly different for each language.</para><para>By default, the text locale is initialized to the system locale (as returned by Locale#getDefault). This assumes that the text to be rendered will most likely be in the user's preferred language.</para><para>If the actual language of the text is known, then it can be provided to the text renderer using this method. The text renderer may attempt to guess the language script based on the contents of the text to be drawn independent of the text locale here. Specifying the text locale just helps it do a better job in certain ambiguous cases</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setTextLocale
				/// </java-name>
				[Dot42.DexImport("setTextLocale", "(Ljava/util/Locale;)V", AccessFlags = 1)]
				public virtual void SetTextLocale(global::Java.Util.Locale locale) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Set the paint's text size. This value must be &gt; 0</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setTextSize
				/// </java-name>
				[Dot42.DexImport("setTextSize", "(F)V", AccessFlags = 257)]
				public virtual void SetTextSize(float textSize) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Set the paint's horizontal scale factor for text. The default value is 1.0. Values &gt; 1.0 will stretch the text wider. Values &lt; 1.0 will stretch the text narrower.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setTextScaleX
				/// </java-name>
				[Dot42.DexImport("setTextScaleX", "(F)V", AccessFlags = 257)]
				public virtual void SetTextScaleX(float scaleX) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				/// <para>Set the paint's horizontal skew factor for text. The default value is 0. For approximating oblique text, use values around -0.25.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// setTextSkewX
				/// </java-name>
				[Dot42.DexImport("setTextSkewX", "(F)V", AccessFlags = 257)]
				public virtual void SetTextSkewX(float skewX) /* MethodBuilder.Create */ 
				{
				}
				...
				/// getTextLocale
				/// </java-name>
				public global::Java.Util.Locale TextLocale
				{
				[Dot42.DexImport("getTextLocale", "()Ljava/util/Locale;", AccessFlags = 1)]
						get{ return GetTextLocale(); }
				[Dot42.DexImport("setTextLocale", "(Ljava/util/Locale;)V", AccessFlags = 1)]
						set{ SetTextLocale(value); }
				}
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				/// <para>Returns the target density of the canvas. The default density is derived from the density of its backing bitmap, or Bitmap#DENSITY_NONE if there is not one.</para><para><para>setDensity(int) </para><simplesectsep></simplesectsep><para>Bitmap::getDensity() </para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				/// <para>Specifies the density for this Canvas' backing bitmap. This modifies the target density of the canvas itself, as well as the density of its backing bitmap via Bitmap.setDensity(int).</para><para><para>getDensity() </para><simplesectsep></simplesectsep><para>Bitmap::setDensity(int) </para></para>        
				/// </summary>
				/// <java-name>
				/// setDensity
				/// </java-name>
				[Dot42.DexImport("setDensity", "(I)V", AccessFlags = 1)]
				public virtual void SetDensity(int density) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				/// <para>Returns the maximum allowed width for bitmaps drawn with this canvas. Attempting to draw with a bitmap wider than this value will result in an error.</para><para><para>getMaximumBitmapHeight() </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				/// <para>Returns the maximum allowed height for bitmaps drawn with this canvas. Attempting to draw with a bitmap taller than this value will result in an error.</para><para><para>getMaximumBitmapWidth() </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				/// <para>Preconcat the current matrix with the specified matrix.</para><para></para>        
				/// </summary>
				/// <java-name>
				/// concat
				/// </java-name>
				...
				/// <summary>
				/// <para>Completely replace the current matrix with the specified matrix. If the matrix parameter is null, then the current matrix is reset to identity.</para><para><b>Note:</b> it is recommended to use concat(Matrix), scale(float, float), translate(float, float) and rotate(float) instead of this method.</para><para><para>concat(Matrix) </para></para>        
				/// </summary>
				/// <java-name>
				/// setMatrix
				/// </java-name>
				...
				public virtual void SetDrawFilter(global::Android.Graphics.DrawFilter filter) /* MethodBuilder.Create */ 
				{
				}
 ...
 /* scope: __dot42__ */ 
    ...
    "", AccessFlags = 9)]
		...
		/// <summary>
		/// <para>PixelXorXfermode implements a simple pixel xor (op ^ src ^ dst). This transformation does not follow premultiplied conventions, therefore this mode <b>always</b> returns an opaque color (alpha == 255). Thus it is not really usefull for operating on blended colors. </para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// decodeStream
				/// </java-name>
				[Dot42.DexImport("decodeStream", "(Ljava/io/InputStream;)Landroid/graphics/Movie;", AccessFlags = 9)]
 ...
 /* scope: __dot42__ */ 
		{
				...
				///        cr_offset = y_size
				///        cb_offset = y_size + c_size</pre></para><para>This format is guaranteed to be supported for camera preview images since API level 12; for earlier API versions, check android.hardware.Camera.Parameters#getSupportedPreviewFormats().</para><para>Note that for camera preview callback use (see android.hardware.Camera#setPreviewCallback), the <b>stride</b> value is the smallest possible; that is, it is equal to:</para><para><pre>stride = ALIGN(width, 16)</pre></para><para><para>android.hardware.Camera.Parameters::setPreviewCallback </para><simplesectsep></simplesectsep><para>android.hardware.Camera.Parameters::setPreviewFormat </para><simplesectsep></simplesectsep><para>android.hardware.Camera.Parameters::getSupportedPreviewFormats </para></para>        
				/// </summary>
				/// <java-name>
				/// YV12
				/// </java-name>
				...
				/// YUV_420_888
				/// </java-name>
				[Dot42.DexImport("YUV_420_888", "I", AccessFlags = 25)]
				public const int YUV_420_888 = 35;
				[Dot42.DexImport("<init>", "()V", AccessFlags = 1)]
 ...
 /* scope: __dot42__ */ 
				{
						/// <summary>
						/// <para>If set, decode methods that take the Options object will attempt to reuse this bitmap when loading content. If the decode operation cannot use this bitmap, the decode method will return <code>null</code> and will throw an IllegalArgumentException. The current implementation necessitates that the reused bitmap be mutable and of the same size as the source content. The source content must be in jpeg or png format (whether as a resource or as a stream). The configuration of the reused bitmap will override the setting of inPreferredConfig, if set. The reused bitmap will continue to remain mutable even when decoding a resource which would normally result in an immutable bitmap.</para><para>You should still always use the returned Bitmap of the decode method and not assume that reusing the bitmap worked, due to the constraints outlined above and failure situations that can occur. Checking whether the return value matches the value of the inBitmap set in the Options structure is a way to see if the bitmap was reused, but in all cases you should use the returned Bitmap to make sure that you are using the bitmap that was used as the decode destination.</para>        
						/// </summary>
						/// <java-name>
						/// inBitmap
						/// </java-name>
						...
						/// inPremultiplied
						/// </java-name>
						[Dot42.DexImport("inPremultiplied", "Z", AccessFlags = 1)]
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// op
				/// </java-name>
				[Dot42.DexImport("op", "(Landroid/graphics/Path;Landroid/graphics/Path$Op;)Z", AccessFlags = 1)]
				public virtual bool Op(global::Android.Graphics.Path path, global::Android.Graphics.Path.JavaOp op) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
				...
				/// op
				/// </java-name>
				[Dot42.DexImport("op", "(Landroid/graphics/Path;Landroid/graphics/Path;Landroid/graphics/Path$Op;)Z", AccessFlags = 1)]
				public virtual bool Op(global::Android.Graphics.Path path, global::Android.Graphics.Path path1, global::Android.Graphics.Path.JavaOp op) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
 ...
 /* scope: __dot42__ */ 
				...
				/// </java-name>
				[Dot42.DexImport("android/graphics/Path$Op", AccessFlags = 16409, Signature = "Ljava/lang/Enum<Landroid/graphics/Path$Op;>;")]
				public sealed class JavaOp
 /* scope: __dot42__ */ 
				{
						/// <java-name>
						/// DIFFERENCE
						/// </java-name>
						[Dot42.DexImport("DIFFERENCE", "Landroid/graphics/Path$Op;", AccessFlags = 16409)]
						public static readonly JavaOp DIFFERENCE;
						/// <java-name>
						/// INTERSECT
						/// </java-name>
						[Dot42.DexImport("INTERSECT", "Landroid/graphics/Path$Op;", AccessFlags = 16409)]
						public static readonly JavaOp INTERSECT;
						/// <java-name>
						/// REVERSE_DIFFERENCE
						/// </java-name>
						[Dot42.DexImport("REVERSE_DIFFERENCE", "Landroid/graphics/Path$Op;", AccessFlags = 16409)]
						public static readonly JavaOp REVERSE_DIFFERENCE;
						/// <java-name>
						/// UNION
						/// </java-name>
						[Dot42.DexImport("UNION", "Landroid/graphics/Path$Op;", AccessFlags = 16409)]
						public static readonly JavaOp UNION;
						/// <java-name>
						/// XOR
						/// </java-name>
						[Dot42.DexImport("XOR", "Landroid/graphics/Path$Op;", AccessFlags = 16409)]
						public static readonly JavaOp XOR;
						private JavaOp() /* TypeBuilder.AddPrivateDefaultCtor */ 
						{
						}
